...
class Test:
    ...
    def process(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        # загрузить объект модели

        classifier = BertClassifier(
            model_path=r'lib/rubert-tiny',
            tokenizer_path=r'lib/rubert-tiny',
            n_classes=2,
            epochs=2,
            model_save_path='/content/bert.pt'
        )
        #загрузить веса 
        trained_model = torch.load(r'lib/bert_model_for_avito_balanced_v2.pt',\
         map_location = torch.device("cpu"))
        classifier.model = trained_model
        # выдать предикшны

        task1_prediction['prediction'] = test['description'].apply(classifier.predict)
